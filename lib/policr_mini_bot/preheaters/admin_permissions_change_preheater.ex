defmodule PolicrMiniBot.AdminPermissionsChangePreheater do
  ...
  - 成员现在的状态是 `kicked` 并且之前的状态是 `left`。备注：将不在群内的用户添加到封禁列表与管理员变化无关。
  """
  ...
  def call(
      ...
      when status_new == "kicked" and status_old == "left" do
    {:ignored, state}
  end
  ...
  def call(%{chat_member: chat_member}, state) do
    ...
    with {:ok, chat} <- ChatBusiness.get(chat_id),
      ...
      text = """
      检测到用户 #{mention(user, anonymization: false, parse_mode: "HTML")} 的管理员权限变化，已自动同步至最新的后台权限中。
      ...
      e ->
        send_message(
          chat_id,
          "检测到用户 #{mention(user, anonymization: false)} 的管理员权限变化，但由于某些原因自动同步后台权限失败了。"
        )
